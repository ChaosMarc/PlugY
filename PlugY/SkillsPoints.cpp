...
void UnassignAllSkillsPoints(Unit* ptChar)
  {
  debug_msg("The ptChar when going into UnassignAllSkillsPoints 0x%X\n", ptChar);
  ...
  while (ptSkill)
    ...
    if (getCharClass(ptSkill) == ptChar->nPlayerClass) //ptSkill->ptSkillBin->charclass
      ...
      if (ptSkill->state == -1)
        ...
        debug_msg("The ptCurChar when leaving out D2SetSkillBaseLevelOnClient in UnassignAllSkillsPoints 0x%X\n", ptChar);
        }
      }
    ptSkill = ptSkill->ptNextSkill;
    }
  debug_msg("The ptChar when leaving out UnassignAllSkillsPoints 0x%X\n", ptChar);
